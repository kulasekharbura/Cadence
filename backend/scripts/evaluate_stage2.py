...
async def evaluate_generation(db, top_k, threshold, q_embeddings, doc_id_map):
    ...
    metrics = {
        ...
        "failures": [],
        "api_calls": {"total": 0, "success": 0, "failed": 0, "429": 0},
        "per_question": []
    }
    
    retrieval_service = RetrievalService(db)
    
    with patch("app.core.config.settings.GEMINI_FALLBACK_MODELS", ""):
        for i, q in enumerate(QUESTIONS):
            query_emb = q_embeddings[q["id"]]
            async def mock_embed(chunks):
                return [{"embedding": query_emb}]
            ...
            with patch("app.core.config.settings.RETRIEVAL_TOP_K", top_k), \
                ...
                try:
                    ...
                    from app.main import app
                    transport = ASGITransport(app=app)
                    async with AsyncClient(transport=transport, base_url="http://test", timeout=45.0) as client:
                        ...
                        if chat_res.status_code == 429 or "RESOURCE_EXHAUSTED" in chat_res.text:
                            metrics["api_calls"]["429"] += 1
                            metrics["api_calls"]["failed"] += 1
                            print("CRITICAL: Received 429 RESOURCE_EXHAUSTED. Stopping evaluation immediately.")
                            break
                    ...
                    if q["is_answerable"]:
                        ...
                        if correct:
                            ...
                            q_result["answer_correctness"] = True
                        ...
                        if valid_citations and len(sources) > 0:
                            ...
                            q_result["citation_correctness"] = True
                        ...
                        if refused and len(sources) == 0:
                            metrics["groundedness"] += 1
                            metrics["no_answer_accuracy"] += 1
                            q_result["no_answer_accuracy"] = True
                            q_result["groundedness"] = True
                            q_result["answer_correctness"] = True
                        else:
                            metrics["failures"].append({
                    ...
                    print(f"Error on question {q['q']}: {e}")
                    if "429" in str(e):
                        metrics["api_calls"]["429"] += 1
                        break
...
async def main():
    try:
        ...
        async with async_session_maker() as db:
            ...
            print(f"Skipping Retrieval Sweep as per Stage 2 restrictions.")
            
            best_config = (5, 0.5)
            print(f"Running Generation for Config top_k=5, threshold=0.5...")
            ...
            with open("phase_13_generation_results.json", "w") as f:
                json.dump(results, f, indent=2)
                
            import csv
            with open("generation_results.csv", "w", newline="") as f:
                writer = csv.writer(f)
                writer.writerow(["ID", "Category", "Answerable", "AnswerCorrect", "CitCorrect", "CitComplete", "NoAnswerAcc", "Groundedness", "ModelUsed"])
                for q_result in gen_metrics.get("per_question", []):
                    writer.writerow([
                        q_result["id"],
                        q_result["category"],
                        q_result["is_answerable"],
                        q_result["answer_correctness"],
                        q_result["citation_correctness"],
                        q_result["citation_completeness"],
                        q_result.get("no_answer_accuracy", False),
                        q_result["groundedness"],
                        q_result["model_used"]
                    ])
